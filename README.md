...
- `Debugger.GlobalManager(global_status=None, typePrint: Optional[str] = "grid"):`
    ...
    - `typePrint=` = Глобальный стиль отображения данных
        ...
        Через GUI [Tkinter](#про-режим-отображения--typeprinttk) `console_debugger/gui/main.pyw` или через TUI [Urwid](#про-режим-отображения--urwid) 
...
- Передвигаться между консолями можно сочетанием клавиш `Cntrl + (Стрелки)` или `Tab`
- Можно отчистить консоль вывода если написать в нижнею консоль `clear`  и нажать `Enter`
